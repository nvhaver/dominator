  ...
  //    parent.addEventListener('DOMNodeInserted',this.fff, true, true);
   ...
   //   parent.addEventListener( "message",this , true, true);
     ...
     addHtmlToElement:function addHtmlToElement(data){
        if(typeof DOMISidebar.loggerWindow.contentDocument=="undefined")
            this.loggerWindow=this.getMainWindow().sidebar.DOMISidebar.loggerWindow//. parent.window.opener.getElementById("logViewer");
          
          DOMISidebar.loggerWindow.contentDocument.body.innerHTML+=data;
     }, 
     getMainWindow: function(){
        var wm = Components.classes["@mozilla.org/appshell/window-mediator;1"]
             ...
             // win is [Object ChromeWindow] (just like window), do something with it
                  if(win.sidebar.DOMISidebar.loggerWindow.contentDocument!==null){
                    return win;             
                  }
                }
     },
     addNodeToElement:function addNodeToElement(data){
        ...
        if(typeof DOMISidebar.loggerWindow.contentDocument=="undefined")
          ...
          var div=DOMISidebar.loggerWindow.contentDocument.createElement("div");
          //dump(data+"\n");
          ...
          DOMISidebar.loggerWindow.contentDocument.body.appendChild(div);
     },
     addNodeToSinkElement:function addNodeToSinkElement(data){
 
        if(typeof DOMISidebar.sinkWindow.contentDocument=="undefined")
            this.sinkWindow=this.getMainWindow().sidebar.DOMISidebar.sinkWindow//. parent.window.opener.getElementById("logViewer");
          
          var div=DOMISidebar.sinkWindow.contentDocument.createElement("div");
          //dump(data+"\n");
          div.style.border="1px solid darkgrey";
          div.innerHTML=data;
          ...
          DOMISidebar.sinkWindow.contentDocument.body.appendChild(div);
     },     
     deleteContent:function  deleteContent(){
          this.hash=null;
          this.lastNode=null;
          DOMISidebar.loggerWindow.contentDocument.body.innerHTML="<style>body{font-size:small}#value{background-color: black;color: pink;}#Getter{background-color:  darkgreen;color: white;}#Flow{background-color: white;color: black;}#Setter{background-color: blue;color: white;}#Sink{background-color: red;color: black}#callstack{border:1px solid}</style>";
          DOMISidebar.sinkWindow.contentDocument.body.innerHTML="<style>body{font-size:small}#value{background-color: black;color: pink;}#Getter{background-color:  darkgreen;color: white;}#Flow{background-color: white;color: black;}#Setter{background-color: blue;color: white;}#Sink{background-color: red;color: black}#callstack{border:1px solid}</style>";
     },
     updateCountElement:function updateCountElement(){
       //dump("aggiorno e basta\n"+this.lastNode+" ----------------- \n");
        
       var iternode= DOMISidebar.loggerWindow.contentDocument.evaluate("//span[@id='callcount']", this.lastNode,null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null);
	...
	var hash = this._md5.finish(false);
     ...
     handleEvent:function handleStorage(ev){
      ...
      //    dump("cancello"+attr_pair[i][0]+" "+attr_pair[i][1] +"\n");
          ...
          var dataObj=eval(unescape(attr_pair[i][1]));
          if(this.isEqualToLast(dataObj))
           this.updateCountElement();
          else{
            var xml=DOMISidebar.dataToNode(dataObj)
            if(xml!=null) {
           try{
